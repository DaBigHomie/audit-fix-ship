...
## Run (from documentation-standards or repo-local if available)

First check if scripts exist locally in your active repo; fall back to documentation-standards:

```bash
REPO_ROOT=$(git rev-parse --show-toplevel)
if [ -f "$REPO_ROOT/scripts/warden-run.mts" ]; then
  cd "$REPO_ROOT"
...
```bash
if [ -f "$REPO_ROOT/scripts/warden-doc-place.mts" ]; then
  npx tsx "$REPO_ROOT/scripts/warden-doc-place.mts" --repo <slug>
else
  cd ~/management-git/documentation-standards
