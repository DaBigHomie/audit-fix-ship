...
- `MGMT_ROOT = ~/management-git` (symlink to `$(git rev-parse --show-toplevel)` when local)
...
- Never hardcode `/Users/dabighomie/` — use `~/management-git/` or `git rev-parse --show-toplevel`
