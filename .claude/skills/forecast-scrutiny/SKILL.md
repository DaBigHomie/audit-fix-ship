...
BLAST_RADIUS = {
  ...
  git_ops:      [none | commit | push | branch | worktree | restore],  # grep the code
...
- **Path/machine** — hardcoded paths? Do they match THIS machine/user? (`$(git rev-parse --show-toplevel)` vs hardcoded paths)
...
- FORECAST: git_ops = **none but read-only `git status`** (no push/branch/worktree);
  reads_from = **`workflow-configs/`** (uses `$(git rev-parse --show-toplevel)` to resolve;
  verify it points to THIS machine's intended path); writes_to = 9 approved repos' main checkout;
...
- SCRUTINY: **name != behavior** — "sync my work" is false; source is `workflow-configs/`, so it
  would NOT distribute the edited DWA `.github`, and could overwrite it. A path that does not resolve on this machine throws "source not found".
