...
First, check if scripts exist in the active repo. If present, use them; otherwise skip:

```bash
REPO_ROOT=$(git rev-parse --show-toplevel)
if [ -f "$REPO_ROOT/scripts/warden.mts" ]; then
  cd "$REPO_ROOT"
  ...
  # Script not available in this repo; refer to documentation-standards
