...
- **Do not** use `git restore` or `git switch` on main branches without explicit user approval.
...
3. Post using **`documentation-standards/scripts/post-handoff-to-slack.mjs`** (repo-local if available):

If script exists locally in active repo:
```bash
cd $(git rev-parse --show-toplevel)
if [ -f scripts/post-handoff-to-slack.mjs ]; then
  ...
  # Fall back to documentation-standards
...
3. **Run** (check if script exists in active repo first; fall back to documentation-standards):

```bash
cd $(git rev-parse --show-toplevel)
if [ -f scripts/slack/post-session-summary.mts ]; then
