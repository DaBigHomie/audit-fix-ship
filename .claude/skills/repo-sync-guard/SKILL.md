...
if [ -f "$REPO_ROOT/scripts/repo-sync-guard.mts" ]; then
  npx tsx "$REPO_ROOT/scripts/repo-sync-guard.mts" .
else
  npx tsx ~/management-git/maximus-ai/.system/scripts/repo-sync-guard.mts .
fi

# refresh remote refs first (read-only on your branches), then audit
if [ -f "$REPO_ROOT/scripts/repo-sync-guard.mts" ]; then
  npx tsx "$REPO_ROOT/scripts/repo-sync-guard.mts" . --fetch
else
  npx tsx ~/management-git/maximus-ai/.system/scripts/repo-sync-guard.mts . --fetch
fi

# audit every git repo under a root
npx tsx ~/management-git/maximus-ai/.system/scripts/repo-sync-guard.mts --root ~/management-git

# machine-readable verdict to STDOUT (never a file)
if [ -f "$REPO_ROOT/scripts/repo-sync-guard.mts" ]; then
  npx tsx "$REPO_ROOT/scripts/repo-sync-guard.mts" . --json
else
  npx tsx ~/management-git/maximus-ai/.system/scripts/repo-sync-guard.mts . --json
fi
```
